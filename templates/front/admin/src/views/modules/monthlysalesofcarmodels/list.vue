	...
	<div class="main-content" :style='{"padding":"60px 30px","margin":"0 0 0 65px","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 10px 0px"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","padding":"10px","flexWrap":"wrap","background":"none","display":"flex"}' >
					<div :style='{"margin":"0 10px 20px 0","display":"flex"}'>
						<label :style='{"border":"1px solid #CBCBCB","padding":"0 10px","margin":"0 10px 0 0","color":"#000","textAlign":"center","display":"inline-block","borderRadius":"10px","background":"#fff","width":"180px","lineHeight":"34px","fontSize":"inherit","fontWeight":"500","height":"34px"}' class="item-label">年份</label>
						<el-input v-model="searchForm.year" placeholder="年份" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 20px 0","display":"flex"}'>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					<el-button class="add" v-if="isAuth('monthlysalesofcarmodels','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","borderRadius":"5px","height":"34px"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('monthlysalesofcarmodels','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"14px","color":"#6EADEF","height":"34px"}'></span>
						删除
					</el-button>

					<el-button class="btn18" v-if="isAuth('monthlysalesofcarmodels','导入')" type="success" @click="importHandler()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						导入
					</el-button>
					<el-button class="btn18" v-if="isAuth('monthlysalesofcarmodels','上传模板')" type="success" @click="importClcik">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						上传模板
					</el-button>
					<el-button class="btn18" v-if="isAuth('monthlysalesofcarmodels','下载模板')" type="success" @click="download('upload/monthlysalesofcarmodels_template.xlsx')">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						下载模板
					</el-button>

					<el-button class="btn18" v-if="isAuth('monthlysalesofcarmodels','车型统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						车型统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('monthlysalesofcarmodels','车型销量数据统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						车型销量数据统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('monthlysalesofcarmodels','厂商销售数据统计')" type="success" @click="chartDialog3()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						厂商销售数据统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('monthlysalesofcarmodels','厂商词云')" type="success" @click="chartDialog4()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						厂商词云
					</el-button>
					<el-button class="btn18" v-if="isAuth('monthlysalesofcarmodels','车型词云')" type="success" @click="chartDialog5()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						车型词云
					</el-button>
				</el-row>
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"14px","color":"#000"}'>
				<el-table class="tables"
					...
					:border='true'
					v-if="isAuth('monthlysalesofcarmodels','查看')"
					:data="dataList"
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('monthlysalesofcarmodels','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="edit" v-if=" isAuth('monthlysalesofcarmodels','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>
							...
							<el-button class="del" v-if="isAuth('monthlysalesofcarmodels','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<el-dialog title="导入" :visible.sync="importVisiable" width="50%">
			<el-form ref="form" :model="form" label-width="80px">
				<el-form-item class="upload" label="文件" prop="excelFile">
				  <excel-file-upload
				  tip="点击上传直接导入excel文件"
				  action="monthlysalesofcarmodels/importExcel"
				  :limit="1"
				  :fileUrls="importUrl"
				  @change="importChange"
				  ></excel-file-upload>
		...
		<el-dialog title="上传模板" :visible.sync="importVis" width="50%">
			<el-form ref="form" :model="importForm" label-width="80px">
				<el-form-item class="upload" label="文件" prop="excelFile">
					<el-upload class="upload-demo" drag :action="$base.url + 'file/upload?type=monthlysalesofcarmodels_template'"
		...
		<el-dialog
			...
			width="800">
			<div id="modelChart1" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="modelChart2" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="shareofsalesChart3" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="shareofsalesChart4" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
		...
		<el-dialog
			...
			width="800">
			<div id="modelChart5" style="width:100%;height:600px;"></div>
			<span slot="footer" class="dialog-footer">
	...
	export default {
		...
		methods: {
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var modelChart1 = echarts.init(document.getElementById("modelChart1"),'macarons');
					this.$http({
						url: "monthlysalesofcarmodels/group/model",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
			...
			chartDialog2() {
				...
				this.$nextTick(()=>{
					...
					var modelChart2 = echarts.init(document.getElementById("modelChart2"),'macarons');
					this.$http({
						url: `monthlysalesofcarmodels/valueMul/model?yColumnNameMul=manufacturer,sales`,
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							titleObj.text = '车型销量数据统计'
							
							const legendObj = this.line.legend
							legendObj.data = [
								'销量(辆)/bar'.split('/').length>1?'销量(辆)/bar'.split('/')[0]:'销量(辆)/bar',
								'售价(万元)'.split('/').length>1?'售价(万元)'.split('/')[0]:'售价(万元)',
							]
							...
							let seriesObj = [
								{
									data: yAxis1,
									type: '销量(辆)/bar'.split('/').length>1?'销量(辆)/bar'.split('/')[1]:'line',
									name: '销量(辆)/bar'.split('/').length>1?'销量(辆)/bar'.split('/')[0]:'销量(辆)/bar',
								},
								{
									data: yAxis2,
									type: '售价(万元)'.split('/').length>1?'售价(万元)'.split('/')[1]:'line',
									name: '售价(万元)'.split('/').length>1?'售价(万元)'.split('/')[0]:'售价(万元)',
								},
							]
			...
			chartDialog3() {
				...
				this.$nextTick(()=>{
					...
					var shareofsalesChart3 = echarts.init(document.getElementById("shareofsalesChart3"),'macarons');
					this.$http({
						url: `monthlysalesofcarmodels/valueMul/shareofsales?yColumnNameMul=manufacturer,sales`,
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							for(let i=0;i<res[0].length;i++){
								...
								xAxis1.push(res[0][i].shareofsales);
								yAxis1.push(parseFloat((res[0][i].total)));
								pArray1.push({
									value: parseFloat((res[0][i].total)),
									name: res[0][i].shareofsales
								})
							}
							let xAxis2 = [];
							let yAxis2 = [];
							...
							for(let i=0;i<res[1].length;i++){
								...
								xAxis2.push(res[1][i].shareofsales);
								yAxis2.push(parseFloat((res[1][i].total)));
								pArray2.push({
									value: parseFloat((res[1][i].total)),
									name: res[1][i].shareofsales
								})
							}
							var option = {};
							let titleObj = this.funnel.title
							titleObj.text = '厂商销售数据统计'
							
							let legendObj = {
								data: xAxis1,
							}
							legendObj = Object.assign(legendObj , this.funnel.legend)
							let tooltipObj = {trigger: 'item',formatter: '{a} <br/>{b} : {c}'}
							...
							for (let i in seriesObj) {
								seriesObj[i] = Object.assign(seriesObj[i] , this.funnel.series)
							}
							...
							// 使用刚指定的配置项和数据显示图表。
							shareofsalesChart3.setOption(option);
							//根据窗口的大小变动图表
							window.onresize = function() {
								shareofsalesChart3.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			chartDialog4() {
				...
				this.$nextTick(()=>{
					...
					var shareofsalesChart4 = echarts.init(document.getElementById("shareofsalesChart4"),'macarons');
					this.$http({
						url: "monthlysalesofcarmodels/group/shareofsales",
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							let pArray = []
							for(let i=0;i<res.length;i++){
								xAxis.push(res[i].shareofsales);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].shareofsales
								})
							}
							var option = {};
							// 使用刚指定的配置项和数据显示图表。
							shareofsalesChart4.setOption(option);
							//根据窗口的大小变动图表
							window.onresize = function() {
								shareofsalesChart4.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			chartDialog5() {
				...
				this.$nextTick(()=>{
					...
					var modelChart5 = echarts.init(document.getElementById("modelChart5"),'macarons');
					this.$http({
						url: "monthlysalesofcarmodels/group/model",
						method: "get",
					}).then(({ data }) => {
			...
			getDataList() {
				...
				let params = {
 				...
 				if(this.searchForm.year!='' && this.searchForm.year!=undefined){
				...
				this.$http({
					url: "monthlysalesofcarmodels/page",
					method: "get",
					params: params
				}).then(({ data }) => {
					if (data && data.code === 0) {
			...
			async deleteHandler(id ) {
				...
				await this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
					...
					await this.$http({
						url: "monthlysalesofcarmodels/delete",
						method: "post",
						data: ids
					}).then(async ({ data }) => {
						if (data && data.code === 0) {
